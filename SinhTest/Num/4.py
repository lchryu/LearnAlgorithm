...
# Sieve of Eratosthenes function to find all primes <= max_n (1e7 in this case)
def sieve(max_n):
...
# Check if all digits of a number are prime digits (2, 3, 5, or 7)
def check_digits(n):
    prime_digits = {'2', '3', '5', '7'}
    return all(digit in prime_digits for digit in str(n))

# Generate the prime list for numbers up to 1e7
max_n = 10**7
primes = sieve(max_n)

# Function to generate random test cases
def generate_test_case(test_number):
    a = random.randint(1, max_n)  # Random number between 1 and 1e7
    b = random.randint(a, max_n)  # Random number between a and 1e7
    input_data = f"{a} {b}\n"
    
    # Calculate the expected output
    count = 0
    for i in range(a, b + 1):
        if primes[i] and check_digits(i):
            count += 1
